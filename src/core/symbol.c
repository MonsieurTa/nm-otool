...
/*   Updated: 2020/02/02 17:46:58 by wta              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		handle_symtab(t_mach_o *mach_o, t_symtab_command *sym)
{
	...
	while (++i < sym->nsyms)
	{
		if (!ptr_valid_range(mach_o->content, mach_o->filestat.st_size, nlist)
		|| !handle_symbol(mach_o, nlist))
			return (0);
...
int		handle_symbol(t_mach_o *mach_o, void *nlist)
{
	...
	if (!ptr_valid_range(mach_o->content, mach_o->filestat.st_size, symname))
		symname = NULL;
	offset = mach_o->is_64 ?
		offsetof(t_nlist_64, n_value) : offsetof(t_nlist, n_value);
	n_value_size = mach_o->is_64 ? sizeof(uint64_t) : sizeof(uint32_t);
