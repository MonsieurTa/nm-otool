...
/*   Updated: 2020/02/09 15:53:28 by wta              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static uint8_t	get_symbol_letter(t_mach_o *mach_o, void *nlist)
{
	uint8_t	n_type;
	uint8_t	tolower;
...
static int		handle_symbol(t_nm *nm, void *nlist)
{
	t_mach_o	*mach_o;
	uint64_t	n_value;
	uint8_t		offset;
...
int				handle_symtab(t_nm *nm, t_symtab_command *sym)
{
	...
	while (++i < sym->nsyms)
	{
		if (!ptr_valid_range(mach_o->content, nm->filestat.st_size,
			nlist + offset))
